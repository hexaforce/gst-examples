...
static gboolean on_webrtcbin_stat(const GQuark fieldname, const GValue *value, gpointer unused) {
  if (GST_VALUE_HOLDS_STRUCTURE(value)) {
    GST_DEBUG("stat: \'%s\': %" GST_PTR_FORMAT, g_quark_to_string(fieldname), gst_value_get_structure(value));
  } else {
    GST_FIXME("unknown field \'%s\' value type: \'%s\'", g_quark_to_string(fieldname), g_type_name(G_VALUE_TYPE(value)));
  }
  return TRUE;
}
...
static gboolean bus_watch_cb(GstBus *bus, GstMessage *message, gpointer user_data) {
  ...
  case GST_MESSAGE_ASYNC_DONE: {
    GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(GST_BIN(pipe1), GST_DEBUG_GRAPH_SHOW_ALL, "webrtc-sendrecv.async-done");
    break;
  }
  case GST_MESSAGE_ERROR: {
    GError *error = NULL;
    gchar *debug = NULL;
    GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(GST_BIN(pipe1), GST_DEBUG_GRAPH_SHOW_ALL, "webrtc-sendrecv.error");
...
static gboolean start_pipeline(gboolean create_offer, guint opus_pt, guint vp8_pt) {
  GstBus *bus;
  ...
  video_desc = g_strdup_printf(
    "videotestsrc is-live=true pattern=ball ! "
    "video/x-raw,width=1024,height=768,framerate=60/1 ! "
    "videoconvert ! "
    "queue ! "
    /* increase the default keyframe distance, browsers have really long periods between keyframes and rely on PLI events on packet loss to fix corrupted video. */
    "vp8enc deadline=1 keyframe-max-dist=2000 ! "
    /* picture-id-mode=15-bit seems to make TWCC stats behave better, and fixes stuttery video playback in Chrome */
    "rtpvp8pay name=videopay picture-id-mode=15-bit pt=%u ! "
    "queue",
    vp8_pt);
  ...
  if (custom_ice) {
    ...
    webrtc1 = gst_element_factory_make_full("webrtcbin", "name", "sendrecv", "stun-server", STUN_SERVER, "ice-agent", custom_agent, NULL);
  } else {
    webrtc1 = gst_element_factory_make_full("webrtcbin", "name", "sendrecv", "stun-server", STUN_SERVER, NULL);
  }
  ...
  if (!create_offer) {
    /* XXX: this will fail when the remote offers twcc as the extension id cannot currently be negotiated when receiving an offer. */
    GST_FIXME("Need to implement header extension negotiation when reciving a remote offers");
  } else {
  ...
  g_signal_connect(webrtc1, "on-negotiation-needed", G_CALLBACK(on_negotiation_needed), GINT_TO_POINTER(create_offer));
...
static void on_offer_received(GstSDPMessage *sdp) {
  GstWebRTCSessionDescription *offer = NULL;
  GstPromise *promise;
  /* If we got an offer and we have no webrtcbin, we need to parse the SDP, get the payload types, then start the pipeline */
  if (!webrtc1 && our_id) {
    ...
    gst_println("Starting pipeline with opus pt: %u vp8 pt: %u", opus_pt, vp8_pt);
...
static void on_server_message(SoupWebsocketConnection *conn, SoupWebsocketDataType type, GBytes *message, gpointer user_data) {
  ...
  if (g_strcmp0(text, "HELLO") == 0) {
    ...
    if (json_object_has_member(object, "sdp")) {
       ...
       * but it's possible to comment out the offer creation and wait for an offer instead, so we handle either here.
       *
       * See tests/examples/webrtcbidirectional.c in gst-plugins-bad for another example how to handle offers from peers and reply with answers using webrtcbin. */
...
int main(int argc, char *argv[]) {
  ...
  GST_DEBUG_CATEGORY_INIT(GST_CAT_DEFAULT, "webrtc-sendrecv", 0, "WebRTC Sending and Receiving example");
  if (!check_plugins()) {
    goto out;
  }
  if (!peer_id && !our_id) {
